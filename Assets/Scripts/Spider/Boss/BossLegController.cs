...
public class BossLegController : MonoBehaviour
{
    ...
    private BossLegJump hop = new BossLegJump();

    void Start()
    {
        target = objective.position;
        hop.Start(curve, hopAltitude, hopDuration);
    }
    ...
    void CheckDistance()
    {
        if (!hop.isMoving())
        {
            float distance = Vector3.Distance(target, objective.position);
            if (distance > maxDistance * transform.lossyScale.x)
            {
                hop.UpdatePositions(target, objective.position);
            }
        }
    }
    ...
    void UpdateIK(Transform currentTransform, Vector3 localTarget)
    {
        ...
        if (ComputeCircleCircleIntersection(localUpperArm2d, upperArmLength, localTarget2d, forearmLength, out p1, out p2))
        {
            ...
            forearm.LookAt(target, shoulder.up);
        } else {
            upperArm.LookAt(target, shoulder.up);
            forearm.LookAt(target, upperArm.up);
        }
    }
    ...
    void LateUpdate()
    {
        ...
        Vector3 localTarget = currentTransform.InverseTransformPoint(target);
